		...
		builder.filter(QueryBuilders.termQuery("productId", productId))
			   ...
			   .execute(new ActionListener<BulkByScrollResponse>() {
				   ...
				   public void onResponse(BulkByScrollResponse response) {
					   ...
					   logger.info("Deleting hashtags for product id {} of index {} succeeded from Elasticsearch.", productId, "product");
					   logger.info("Deleted hashtags number : {}.", deletedNum);
				   }

				   @Override
				   public void onFailure(Exception e) {
					   logger.error("An Exception occurred while trying to delete hashtags. Exception : {}", e.toString());
					   logger.error("Manual deletion required. Hashtags must be deleted : product id : {} of index {}."
							   																				, productId, "product");
