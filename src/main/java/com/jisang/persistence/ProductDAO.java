...
public interface ProductDAO extends MybatisMapper {
	...
	/**
	 * 해시태그와 상품 이미지는 PRODUCT 도메인을 구성하는 정보이다. 하나의 PRODUCT에 대하여 여러 개의 해시태그와 이미지가 존재하므로 이들 정보에 대한 테이블을 따로 
	 * 빼두었다. 아직까지는 해시태그와 이미지(이미지가 저장된 storage상의 url)가 PRODUCT를 구성하는 데이터 이상의 의미를 갖지는 않기 때문에 따로 도메인으로 취급하지는
	 * 않았으며 그러므로 PRODUCT 도메인에 한 data access를 수행 로직이 담긴 이 DAO 인터페이스에 해시태그와 이미지(url)에 대한 입력 메서드를 정의하였다. 
	 */
