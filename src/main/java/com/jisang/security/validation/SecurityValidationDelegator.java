 ...
 * 
 * com.jisang.security와 그 하위의 패키지 컴포넌트들을 후에 다시 사용할 일이 있을 경우를 대비하여
 * com.jisang.security 외부와 최대한 분리시키려는 개인적인 목적 때문에 com.jisang.validation에도
 * {@link ValidationDelegator} 클래스가 있으나 이 클래스를 사용하지 않았다. 또한
 * {@link ValidationDelegator} 클래스와 다른 점이라곤 던져지는 예외에 타입 하나뿐인데,
 * {@link ValidationDelegator}를 계승하여 오버라이딩하지 않은 이유도 위의 설명한 목적 때문이다.
 * 
 * 
 * 
 * @author leeseunghyun
