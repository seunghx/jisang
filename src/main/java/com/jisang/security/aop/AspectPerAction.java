...
/**
 * 
 * com.jisang.security 하위의 클래스 중 특정 동작을 취하는 클래스의 메서드에 적용될 Aspect 구현 클래스이다. 여기서
 * 말하는 특정 동작이란 각각의 메서드 단위가 아닌 인증 작업, 혹은 JWT token building 작업 등 하나 이상의 메서드의
 * 콜래보레이션 수준의 동작을 말한다.
 * 
...
public class AspectPerAction {
    ...
    /**
     * 
     * 인증 작업은 하나의 큰 작업이라고 할 수 있다. 이런 작업의 시작과 끝을 알리기 위해 작성된 로깅 애스팩트이다. 하나의 (하나의 메서드
     * 호출보다는)큰 작업 흐름에 대한 로깅이기 때문에 로깅 레벨은 INFO로 정하였다.
     *
     */
    ...
    /**
     * 
     * JWT token parsing 작업의 시작과 끝을 알리기 위해 작성된 로깅 애스팩트이다. 사실 JWT token parsing은 인증
     * 작업의 한 과정일 뿐이라고 볼 수도 있으나 JWT token 인증 기반으로 동작하는 어플리케이션이기 때문에 기타 메서드 호출들보다는
     * 중요하다고 생각하여 {@link AbstractJWTService#parseToken} 메서드에 대한 INFO 레벨 로깅 애스팩트를
     * 추가하게 되었다.
     * 
    ...
    public TokenDTO parseJWTTokenExceptionTranslationAndLogging(ProceedingJoinPoint jp) throws Throwable {
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                ...
                logger.info("{}#{} throws IllegalArgumentException.", jp.getTarget(),
            ...
            logger.info("Parsing JWT token failed.");
            throw new InternalAuthenticationServiceException("An System exception occurred while trying to parse JWT.",
                    e);
        }
    }
    ...
    /**
     * 
     * JWT token building 작업의 시작과 끝을 알리기 위해 작성된 로깅 애스팩트이다. 사실 JWT token building은 인증
     * 작업의 한 과정일 뿐이라고 볼 수도 있으나 JWT token 인증 기반으로 동작하는 어플리케이션이기 때문에 기타 메서드 호출들보다는
     * 중요하다고 생각하여 {@link AbstractJWTService#buildToken} 메서드에 대한 INFO 레벨 로깅 애스팩트를
     * 추가하게 되었다.
     * 
     *
     */
    ...
    public String buildJWTTokenExceptionTranslationAndLogging(ProceedingJoinPoint jp) throws Throwable {
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                ...
                logger.info("{}#{} throws SignatureException while trying to set SignatureAlgorithm.", jp.getTarget(),
                        jp.getSignature().toShortString());
            ...
            if (logger.isInfoEnabled()) {
                logger.info("Building JWT failed.");
                logger.info("{}#{} throws CompressionException  while trying to compress JWT token.", jp.getTarget(),
                        jp.getSignature().toShortString());
            ...
            logger.info("Building JWT failed.");
            throw new InternalAuthenticationServiceException("An System exception occurred while trying to build JWT.",
                    e);
        }
    }
    ...
    /**
     * 
     * 클라이언트에 인증 번호를 전송하는 {@link AuthenticationNumberNotificationProvider} 구현 클래스에
     * 적용될 로깅 애스팩트.
     *
     */
    ...
    /**
     * 
     * jisang 어플리케이션의 security 단에서의 인증 과정 중 data access layer에서 발생한
     * {@link DataAccessException} 타입 예외를 {@link AuthenticationFailureHandler} 등에서
     * 처리할 수 있도록 {@link AuthenticationServiceException}으로 변환시킨다.
     * 
     */
    @AfterThrowing(value = "execution(* com.jisang.security.persistence.*.*(..))", throwing = "ex")
    public void authenticationProccessingDataAccessExceptionTranslation(JoinPoint jp, DataAccessException ex)
            throws RuntimeException {

        if (logger.isInfoEnabled()) {
            logger.info("Exception occurred in Data Acess Layer - {}#{}", jp.getTarget(),
                    jp.getSignature().toShortString());
