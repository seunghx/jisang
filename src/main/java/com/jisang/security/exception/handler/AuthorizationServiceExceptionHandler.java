...
/**
 * 
 * {@link AuthorizationServiceException}을 처리하는 {@link SecurityExceptoinHandler}
 * 구현.
 * 
 * {@link AuthorizationServiceException} 타입 예외는 인가 처리 중에 발생한
...
public class AuthorizationServiceExceptionHandler
    ...
    /**
     * 
     * 이 메서드 구현은 단순히 500 - InternalServerError HTTP 응답을 나타내는
     * {@link SimpleResponseDTO}를 반환한다. 항상 같은 응답 status code와 message를 갖고 있기 때문에 반환용
     * {@link SimpleResponseDTO}를 프로퍼티로 갖고 이를 반환하는 방법도 있겠지만, 이러한 방법은 외부에서 반환된 DTO의
     * 내부를 바꿀 위험이 있기 때문에 이 메서드에서 반환할 객체를 매번 생성해서 반환하는 방법을 택하였다.
     * 
     */
    @Override
