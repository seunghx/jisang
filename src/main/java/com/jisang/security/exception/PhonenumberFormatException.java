...
/**
 * 
 * 잘못 입력된 핸드폰 번호가 발견 되었을 때 이 예외가 던져진다. DB에 저장된 유저의 핸드폰 번호와 다른 핸드폰 번호가 전달되었을 때 발생하는 
 * {@link InvalidPhoneNumberException}과는 다르다.
 * 
 * 이 예외는 HTTP Status - 400 Bad Request에 해당되는 반면 {@link InvalidPhoneNumberException}은 HTTP Status - 401 - Unauthorized에 해당된다.
 * 
 * @author leeseunghyun
 *
 */
