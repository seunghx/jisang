...
/**
 * 
 * 사용자의 인증 번호 전송과 관련된 인증 정보 {@link AuthenticationNumberTokenDTO} 오브젝트를 담고있는 {@link Authentication} 구현이다.
 * 
 * 단순히 {@link AuthenticationNumberTokenDTO} 전달 하는 일 외에 다른 메서드들은 지원하지 않는다. 스프링 시큐리티 {@code Authentication Filter}의
 * {@code @SuccessfulAuthentication} 메서드의 인자 타입이 {@link Authentication}이기 때문에 마음에 들지는 않지만 이런 구현을 하게 되었다.
 * 
 * 사실 위의 이유로 익명 사용자에 대한 인증(예 : 인증 번호를 통한 인증)은 spring security의 {@code Authentication Filter}가 아닌 서블릿 {@link Filter} 
 * 구현이나 controller에서 수행할까도 생각해보았지만 일관성있게 인증과 관련된 부분은 모두 spring security의 {@code Authentication Filter}를 사용하기로
 * 하였다.
 * 
 * @author leeseunghyun
 *
