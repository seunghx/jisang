...
public class DefaultUserDetailsService implements UserDetailsService {
     ...
     *             데이터베이스 시스템에서 예외가 발생할 경우 이 예외가 던져진다.
     * @throws UsernameNotFoundException
     *             전달받은 {@code username}에 해당하는 유저 정보가 없을 경우 이 예외가 던져진다.
     * @throws InternalAuthenticationServiceException
     *             {@link SecurityValidationDelegator#validate}
     *
     */
    @Override
    public DefaultUserDetails loadUserByUsername(String username) {
        ...
        Account account = Optional.ofNullable(userDAO.find(username))
                .orElseThrow(() -> new UsernameNotFoundException("Username does not exist. username : " + username));
