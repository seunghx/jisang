...
/**
 *
 * 어플리케이션의 유저 인증과 관련하여 사용 될 유저 정보, 그리고 이와 더불어 JWT token 구성에 필요한 정보를 담은
 * {@link TokenDTOBasedUserDetails} 구현이다. 내부적으로 {@link UserAuthTokenDTO} 타입 객체를
 * 담고 있는데 그 이유는 {@link TokenDTOBasedUserDetails}에 설명되어 있다.
 * 
 * @author leeseunghyun
 * 
 */
public class UserAuthTokenDTOBasedUserDetails implements TokenDTOBasedUserDetails {
    ...
    /**
     * {@link GrantedAuthorityMapper}를 이용하여 {@code tokenDTO}가 포함하는 {@link Account}
     * 오브젝트의 role 정보에 해당하는 {@link Collection<? extends GrantedAuthority>}를 반환.
     * 
     * @throws IllegalArgumentException
     *             {@link GrantedAuthorityMapper#resolveAuthority()}
     * 
     */
    @Override
