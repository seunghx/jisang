...
/**
 * 
 * 인증 번호 전송 방식은 보통 사용자의 핸드폰에 문자로 인증 번호를 전송하고 이를 검사하는 방법이 자주 사용되는 것 같다. 다른 방법으로는
 * 푸시 알림으로 인증 번호를 전달한 후 사용자의 핸드폰에 전화연결을 하여 확인하는 방법도 있는 것 같다. 이메일로 인증 번호를 전송할 수도
 * 있을 것이다. 무엇이 되었건 인증 번호를 받으려면 인증 번호가 전송될 목적지에 대한 인증이 필요하다. 예를 들어 SMS로 인증 번호를
 * 받아야 할 경우 사용자의 핸드폰 번호가 목적지가 될 것이다. 전달 받은 유저 이메일(id) 을 이용해 데이터베이스로부터 목적지 정보를
 * 가져온 후 이를 요청 파라미터로 전달된 목적지 정보와 비교해야한다.
 * 
 * 
 * (이 클래스가 인증 번호를 발급하는 필터라면 인증 번호를 인증하는 필터
 * {@link AuthenticationNumberAuthenticationFilter}가 있으며 임시 비밀번호를 발급하는 필터도 있다.
 * 인증 번호를 인증하는 필터 {@link AuthenticationNumberAuthenticationFilter} 클래스에서 바로 임시
 * 비밀번호를 발급하지 않는 이유는 후에 임시 비밀번호 발급 외에도 인증 번호의 인증이 필요한 시나리오가 또 생길 지도 모르기 때문에 인증
 * 되었음만을 알려야 후에 다른 시나리오에서도 사용할 수 있기 때문이다.)
 * 
 * 
 * 
 * 
...
public class AuthenticationNumberEntryPointFilter extends AbstractAuthenticationProcessingFilter {
    ...
    /**
     * 인증 된 사용자에게 응답할 JWT token을 생성. 생성된 JWT token에는 인증 번호가 담겨 있다. 이는 후에 인증 번호에 대한
     * 검사에 사용된다.
     */
    @Autowired
    ...
    /**
     * 
     * 임시 비밀 번호 발급 전 입력 된 정보(이메일, 핸드폰 번호)가 올바른지 검사가 필요하다 이
     * {@link #attemptAuthentication} 구현은 {@link JWTUsernamePasswordAuthentication}
     * 오브젝트를 {@link AuthenticationProvider}에 전달하며 실제 인증 작업을 위임한다.
     * 
     ...
     *             HTTP 401 응답으로 처리되어지는 {@link UsernameNotFoundException,
     *             BadCredentialsException}을 던지도록 하였다.
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        if (StringUtils.isEmpty(notificationEndPoint)) {
            ...
            throw new BadRequestParameterDetectedException("Destination parameter missed.",
                    NOTIFICATION_DESTINATION_PARAMETER, notificationEndPoint);
        }
     ...
     * {@link SecurityRestResponseHelper}를 이용하여 HTTP 응답 헤더에 전달한다. 이 메서드가 호출된다는 것은 인증
     * 번호를 전송할 목적지가 올바르다는 의미이므로 FilterChain을 더 거칠 필요가 없어 바로 응답을 한다. 이
     ...
     * 성공 후 처리 도중에 발생한 예외이기 때문에 이를 {@link InternalAuthenticationServiceException}으로
     * 변환하여 {@link #unsuccessfulAuthentication} 메서드에 전달한다.
     * 
     * 
     * @param authentication
     *            - 인증 성공을 나타내는 {@link Authentication} 객체이다.
     * 
     * 
     */
    @Override
     ...
     * 
     * @param failed
     *            - {@link #attemptAuthentication} 메서드에서 발생한 예외 객체이다. 해당 예외 객체는
     *            {@link SecurityExceptionHandler}에 의해 처리된다.
     * 
     */
    @Override
