...
/**
 * 
 * (비밀 번호를 기억못하는)익명 사용자의 임시 비밀번호 발급 요청에 대한 인증을 수행하는 {@link AbstractAuthenticationProcessingFilter} 구현(추상 클래스 상속)이다.
 * 익명 사용자에 대한 인증을 위해 이 클래스의 필드 {@code jwtService}에서 내부적으로 사용하는 {@link io.jsonwebtoken.JwtParser#parsetClaimsJws()} 메서드는 
 * JWT token의 파싱 작업 중에 토큰의 검증까지 수행한다. 그래서 이 클래스는 따로 {@link AuthenticationProvider} 구현에 인증을 맡기지 않는다. 인증에 성공할 경우 
 * 임시 비밀번호를 생성하여 사용자에게 전달하며 (현재는 이메일을 통해 전달.) 해당 사용자의 데이터베이스 상의 비밀번호 정보를 생성된 임시 비밀번호로 변경한다.
 * 
 * @author leeseunghyun
 *
 */
public class TemporaryPasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	...
	/**
	 * 임시 비밀번호 발급 요청에 대한 익명 사용자(로그인 하지 않은 사용자)의 요청을 인증한다. 인증은 {@code JWTService#parseToken} 메서드를 통해 진행된다.
	 * 위에서 말한대로 이 메서드는 내부적으로 {@link io.jsonwebtoken.JwtParser#parseClaimsJws} 메서드를 사용하는데 이 메서드에는 파싱 작업과 함께 
	 * 토큰의 검증 작업까지 이루어진다.
	 */
	@Override
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		return new AnonymousUserAuthentication(
							(AnonymousUserAuthTokenDTO)jwtServiceResolver.resolveJWTService(tokenDTO.getClass()).parseToken(tokenDTO));
