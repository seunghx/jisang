 ...
 * 
 * 올바른 인증 번호를 전달한 사용자에게 응답을 할 때 또 다른 JWT token의 전송이 필요하다. 인증 번호 검사에 성공한 사용자가 새로운 요청을 할 경우 
 * (예를 들면 "임시 비밀번호 발급"과 같은 버튼 클릭) 해당 요청이 인증된(인증 번호 검사에 성공한) 사용자로부터 전송 되었다는 것을 알려야하기 때문이며 또한
 * 방금 언급한 "임시 비밀번호 발급" 시나리오의 경우 임시 비밀번호를 발급할 사용자의 신원(id)를 알아야 핸드폰이나 이메일 등에 임시 비밀번호를 전송할 수 있기 
 * 때문에 사용자의 신원(id)정보가 담긴 JWT token이 꼭 필요하다.
 * 
 * 처음엔 임시 비밀번호 발급만을 위하여 이 클래스를 정의하려 하였다. 그렇기 때문에 초기의 구상에서는 요청이 인증 번호 검사에 성공할 경우 이 클래스에서
 * 바로 사용자의 이메일에 임시 비밀번호를 전송한다. 그러나 후에 인증 번호 검사가 필요한 또 다른 시나리오가 생길 수 있기 때문에 이 클래스에서 임시 비밀번호를 
 * 보내버리면 재사용성 측면에서 아쉬움이 있을 것이다. 
 * 
 * 위와 같은 이유로 이 클래스는 인증 번호 검사에 성공할 경우 인증 되었음과 유저의 신원(id)를 알리기 위한 또 다른 JWT 토큰을 발급한다.  
 *
 * 
 * @author leeseunghyun
 *
...
public class AuthenticationNumberAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	...
	/**
	 * 
 	 * 인증 번호에 대한 인증을 시도한다. 이 {@code attemptAuthentication()} 구현은 따로 {@link AuthenticationProvider}에 인증을 맡기지 
 	 * 않고 {@link JWTService}의 JWt token parsing 중에 수행되는 검증 방식을 이용해 인증 번호를 검사한다.
	 * 	
	 */
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		return new AuthNumberTokenDTOAuthentication((AuthenticationNumberTokenDTO)jwtService.parseToken(authNumberTokenDTO));
	}
	
	/**
	 * 인증 번호 인증에 성공하였을 경우에 호출되는 메서드이다. 인증번호 검사에 성공했음을 알리는 응답을 보낸다.
	 * 이 클래스의 인자 {@code authentication}의 {@code getPrincipal} 메서드의 반환값을 바로 형변환해도 안전한 이유는 {@code authentication}은
	 * 이 클래스의 메서드 {@link #attemptAuthentication}에서 반환한 {@link AuthNumberTokenDTOAuthentication} 오브젝트이기 때문이다.
	 */
	@Override
