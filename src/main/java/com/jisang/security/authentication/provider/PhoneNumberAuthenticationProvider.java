...
/**
 * 
 * 사용자의 핸드폰에 인증 번호를 전송하기 전 사용자가 입력한 핸드폰과 이메일 정보가 동일한 유저의 핸드폰/이메일 정보인지를 검사할 필요가
 * 있다. 이 {@link AuthenticationProvider} 구현은 위의 작업을 수행한다.
 * 
 * @author leeseunghyun
 *
 */
...
public class PhoneNumberAuthenticationProvider implements AuthenticationProvider {
     ...
     *            {@link UsernameEndPointAuthentication} 타입 오브젝트이다.
     * 
     * @throws InternalAuthenticationServiceException
     *             전달받은 {@code authentication} 객체가 담고 있는 유저 아이디, 핸드폰 번호가 null 또는
     *             빈문자열일 경우 발생.
     * @throws InvalidPhonenumberException
     *             요청된 유저 아이디 정보에 해당하는 유저가 없거나 핸드폰 번호가 일치하지 않을 경우 던져진다.
     * 
     */
    @Override
    public UsernameEndPointAuthentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        String phoneNumberFromDB = Optional.ofNullable(userDAO.findPhoneNumber(username))
                .orElseThrow(() -> new InvalidPhonenumberException("Username does not exist. username : " + username));
        String phoneNumberFromRequest = null;
        try {
            phoneNumberFromRequest = phoneNumberFormatter.parse((String) authentication.getCredentials(),
                    (Locale) authentication.getDetails());
        ...
        if (!phoneNumberFromRequest.equals(phoneNumberFromDB)) {
            if (logger.isDebugEnabled()) {
                ...
                logger.debug("Expected phone number to be : {}, but was {}.", phoneNumberFromDB,
        ...
        return new UsernameEndPointAuthentication(username, phoneNumberFromRequest,
                (Locale) authentication.getDetails());
    }
     ...
     * {@link UsernameEndPointAuthentication}을 {@code supports} 한다고 하였으므로 이 메서드에 전달된
     * {@code authentication} 오브젝트는 당연히 {@link UsernameEndPointAuthentication}일 것이라는
     * 가정하에 이 메서드 코드에 존재하는 타입 변환은 안전하다고 생각하였다.
     * 
     */
    private void validateAuthentication(Authentication authentication) {
        ...
        if (StringUtils.isEmpty((String) authentication.getPrincipal())) {
            throw new InternalAuthenticationServiceException(
                    "Illegal argument detected, user email must not be empty String.");
        }

        if (StringUtils.isEmpty((String) authentication.getCredentials())) {
            throw new InternalAuthenticationServiceException(
                    "Illegal argument detected, user phone must not be empty String.");
        }

        if (!(authentication.getDetails() instanceof Locale)) {
            throw new InternalAuthenticationServiceException(
                    "Illegal argument detected, authentication#getDetails() must return java.util.Locale");
        }
    }
     ...
     * 구현 오브젝트인지를 검사한다. 정확히는 휴대폰 인증에 앞서 필요한 정보인 유저 아이디, 핸드폰 번호를 담고 있는
     * {@link UsernameEndPointAuthentication} 타입일 경우 지원 가능함에 대한 의미로 true를 반환한다.
     * 
     */
    @Override
