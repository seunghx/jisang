...
public class JWTLoginAuthenticationProvider implements AuthenticationProvider {
	 ...
	 * 					 		 {@link JWTAuthentication}을 구현한 {@link JWTUsernamePasswordAuthentication} 타입 오브젝트이다. 
	 * @return  인증에 성공하였을 경우 내부적으로 {@link UserAuthTokenDTO} 오브젝트를 담고 있는 {@link UserAuthTokenDTOAuthentication}을 반환.
	 * 
	 * @throws  InternalAuthenticationServiceException  전달받은 {@link Authentication} 객체가 담고 있는 유저 아이디, 패스워드가 
	 * 			null 또는 빈문자열일 경우 발생.
	 * @throws  BadCredentialsException 요청된 계정 정보의 비밀번호가 올바르지 않을 경우 던져진다.
	 * @throws  UsernameNotFoundException 요청 파라미터(유저 아이디)에 해당하는 유저 정보가 없을 경우 던져진다.
	 * 			 
	 */
	@Override
	 ...
	 * {@link JWTUsernamePasswordAuthentication}을 {@code supports} 한다고 하였으므로 이 메서드에 전달된 {@code authentication} 오브젝트는
	 * 당연히 {@link JWTUsernamePasswordAuthentication}일 것이라는 가정하에 이 메서드 코드에 존재하는 타입 변환은 안전하다고 생각하였다.
	 * 
	 */
	private void validateAuthentication(Authentication authentication) {
		if(StringUtils.isEmpty((String)authentication.getPrincipal())) {
			throw new InternalAuthenticationServiceException("Illegal argument detected, username must not be empty String.");
		}
		if(StringUtils.isEmpty((String)authentication.getCredentials())) {
			throw new InternalAuthenticationServiceException("Illegal argument detected, password must not be empty String.");
		}
	}
	 ...
	 * 정확히는 폼 인증에 필요한 정보인 유저 아이디, 패스워드를 담고 있는 {@link JWTUsernamePasswordAuthentication} 타입일 경우
