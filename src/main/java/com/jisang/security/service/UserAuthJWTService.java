 ...
 * <p>
 * 
 * JWT token을 사용하는 OAuth 등의 프로토콜 시나리오에는 두 종류의 token이 존재한다. 하나는 access-token으로
 * 자원에 접근할 권한을 포함하는 token이며 다른 하나는 refresh-token으로 최초의 인증 서버로부터의 인증에 성공하였을 때
 * access-token과 함께 제공받는 token이다. access-token은 비교적 수명이 짧으며 수명이 다한 access-token은
 * refresh-token에 의해 갱신되기 때문에 refresh-token은 일반적으로 수명이 길다.(무한한 수명을 갖는다는 시나리오도 본
 * 적이 있는데 이는 좋지 않다고 생각한다. 혹시라도 공격자가 refresh-token을 취득하게 될 경우 길더라도 수명을 유한하게 해주어야
 * 취득 된 refresh-token을 무한히 사용하지 않게 할 수 있기 때문이다.)<br>
 * <br>
 * 
 ...
 * 검사를 수행하는 것으로 절충 하였었는데, 이 또한 마음에 들지는 않아 redis를 도입하여 어플리케이션에 어느 정도의 state(JTI)를
 * 추가하기로 하였다.(그나마 다른 정보, user id, user role 등의 상태 유지 정보는 JWT token에 보관된다.) 현재는
 ...
 * 하면 된다. 그러나 이 방법은 jjwt {@link io.jsonwebtoken}을 이용해서는 불가능하다. 이유는 다음과 같다.<br>
 * <br>
 * 
 * {@link io.jsonwebtoken.JwtParser} 구현의 parsing 메서드들은 토큰 만료 기한이 지난 토큰을 전달받을 경우
 * 바로 예외를 던져버려 만료된 토큰에 대한 user id 등에 접근할 방법이 전혀 없다. 즉, 삭제할 redis key 값을 구할 수 없다.
 ...
 * 좋다.)
 * 
 * 그래서 생각한 다음 방법은 아래와 같다. 매 인증 때 JTI정보에 접근할 때마다 해당 JTI 정보를 담는 redis key의 만료 기한을
 * 늘려주는 것이다. 위 방법의 JWT token의 만료 기간을 매번 늘려주는 것과 결국 같은 이치이다. 어플리케이션의 장기간 미사용 유저의
 * redis key는 만료되버릴 것이다. redis에는 key에 대한 만료 기한을 설정하는 기능이 이미 존재하기 때문에(EXPIRE와 같은)
 ...
 * if 문을 사용해 비교하여야 하는 불편이 마음에 들지 않았다. 그래서 결국 두 번째 방법 Redis의 Expired key 정책을
 * 사용하기로 하였다.
 * 
 ...
 * 이용하던 유저에게 갑자기 로그인을 해야한다고 알리는 상황이 발생할 수 있다. <br>
 * <br>
 * 
 * 
 * 아무튼 위의 방법들 중 아래의 방법을 선택함으로서 장시간 미이용 고객에 대한 정보 삭제 방법을 간구하게 되었다. 또한 JWT token의
 * 만료 기한(+ Redis 키 만료 기한) 이 한 곳에만 존재할 수 있도록 이 클래스의 {@link #buildToken}에 의해 호출되는
 * primitive 메서드인 {@link #getClaims} 메서드는 registered claim "exp"를 설정하지 않는다. 또한
 * 위에서도 말하였지만 JWT token이 스스로 만료 기한을 담고 있으면 {@link io.jsonwebtokn} 상에서는 재로그인 말고는
 * 만료된 토큰에 대한 재발급이 불가능하기 때문이기도 하다. <br>
 * <br>
 * 
 * 
