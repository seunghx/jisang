...
public class ModelMapperConfig {
    ...
    public ModelMapper modelMapper() {
    	...
    	modelMapper.createTypeMap(SignupManagerDTO.class, Market.class)
    			   .addMapping(SignupManagerDTO::getMarketName, Market::setName);
    
    	modelMapper.createTypeMap(Address.class, MarketModifyResponseDTO.class)
    			   .addMapping(Address::getId, MarketModifyResponseDTO::setAddress);
    ...
    private Converter<Location, MallMapResponseDTO> locationToMapDTOConverter() {
    	return new AbstractConverter<Location, MallMapResponseDTO>() {
    		private final ModelMapper modelMapper = new ModelMapper();
    		
    		@Override
			protected MallMapResponseDTO convert(Location location) {
    			...
    			if(Objects.isNull(addresses) || addresses.isEmpty()) {
    				throw new IllegalArgumentException("Source object with invalid property.");
    			}
    			
    			List<MapAddressResponseDTO> addressDTOList = new ArrayList<>();
    			
    			addresses.stream().forEach(addr -> {
    				addressDTOList.add(modelMapper.map(addr, MapAddressResponseDTO.class));
    			});
    			
    			mapDTO.setAddressList(addressDTOList);
    			
    			// Set degree
    			mapDTO.setTopLeftLATDegree((int)location.getTopLeftLAT());
    			...
    			// Degree to seconds
    			double topLeftLATSec = (location.getTopLeftLAT() - (int)location.getTopLeftLAT()) * 3600;
    		 	double topLeftLNGSec = (location.getTopLeftLNG() - (int)location.getTopLeftLNG()) * 3600;
    			double bottomRightLATSec = (location.getBottomRightLAT() - (int)location.getBottomRightLAT()) * 3600;
    			double bottomRightLNGSec = (location.getBottomRightLNG() - (int)location.getBottomRightLNG()) * 3600;
    			
    			// Set seconds
    			mapDTO.setTopLeftLATSec(Double.parseDouble(String.format("%.2f", topLeftLATSec)));
    			mapDTO.setTopLeftLNGSec(Double.parseDouble(String.format("%.2f", topLeftLNGSec)));
    			mapDTO.setBottomRightLATSec(Double.parseDouble(String.format("%.2f", bottomRightLATSec)));
    			mapDTO.setBottomRightLNGSec(Double.parseDouble(String.format("%.2f", bottomRightLNGSec)));
