...
@ComponentScan(basePackages= { "com.jisang.security.validation", "com.jisang.security.aop", "com.jisang.security.authentication.provider",
							   "com.jisang.security.authentication.handler",
...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http
			...
			.and()
			.authorizeRequests()
				.antMatchers(ACCESS_MANAGE_MARKET_ANT_PATTERN).access("hasRole('ROLE_MANAGER')")
				.antMatchers(ACCESS_MANAGE_PRODUCT_ANT_PATTERN).access("hasRole('ROLE_MANAGER')");
	}
	...
	public AuthenticationNumberEntryPointFilter authenticationEntryPointFilter() throws Exception {
		AuthenticationNumberEntryPointFilter filter = new AuthenticationNumberEntryPointFilter(authNumberEntryPointRequestMatcher());
	...
	public AuthenticationNumberAuthenticationFilter authenticationNumberAuthenticationFilter() throws Exception {
		AuthenticationNumberAuthenticationFilter filter 
									= new AuthenticationNumberAuthenticationFilter(authNumberAuthenticationRequestMatcher());
	...
	public TemporaryPasswordAuthenticationFilter temporaryPasswordAuthenticationFilter() throws Exception {
		TemporaryPasswordAuthenticationFilter filter
							= new TemporaryPasswordAuthenticationFilter(temporaryPasswordAuthenticationRequestMatcher());
	...
	private RequestMatcher authNumberEntryPointRequestMatcher() {
		RequestMatcher reqMatcher = new AntPathRequestMatcher(AUTHENTICATION_NUMBER_END_POINT, HttpMethod.GET.toString());
		return reqMatcher;
	}
	
	private RequestMatcher authNumberAuthenticationRequestMatcher() {
		RequestMatcher reqMatcher = new AntPathRequestMatcher(AUTHENTICATION_NUMBER_END_POINT, HttpMethod.POST.toString());
		return reqMatcher;
	}
	
	private RequestMatcher temporaryPasswordAuthenticationRequestMatcher() {
		RequestMatcher reqMatcher = new AntPathRequestMatcher(TEMPORARY_PASSWORD_END_POINT, HttpMethod.PATCH.toString());
		return reqMatcher;
	}
