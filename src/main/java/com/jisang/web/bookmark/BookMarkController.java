...
public class BookMarkController {
	...
	/**
	 * 방문한 마켓에 대해 즐겨찾기가 설정 되었는지에 대한  정보를 반환한다. 유저가 해당 마켓에 대하여 기존에 즐겨찾기를 설정하였다면 HttpStatus.OK 상태 코드가 반환될 것이며 
	 * 그렇지 않다면 즐겨찾기 설정을 하지 않았다는 의미로 {@link HttpStatus.NOT_FOUND}가 반환된다.
	 * 
	 * 
	 * 파라미터 {@code userId}는 seucirty 단과 {@link UserIDArgumentResolver}를 통해 null 검사가 수행되기 때문에 안전하다고 판단하고 
	 * 여기선 따로 같은 동작을 반복하지는 않는다. 만약 무언가 잘못되어 null 값의 파라미터 {@code userId}가 전달 된다고 하여도 메서드의 시작부에서 
	 * {@link NullPointerException}이 발생할 것이므로 잘못된 정보가 계속 전달되지는 않을 것이다.
	 * 
	 *
	...
	public ResponseEntity<List<BookMarkInfoDTO>> getBookMarks(UserID userId){
		...
		if(logger.isDebugEnabled()) {
			logger.debug("Calling {} to get bookmark info for user id : {} started.", bookMarkService, userId.getUserId());
		}
	...
	/**
	 * 해당 마켓에 대한 즐겨찾기 설정을 해제한다. 지상 어플리케이션의 와이어 프레임상의 즐겨찾기 목록 화면에 있는 x 버튼을 누를 경우 이 핸들러 메서드가 호출된다.
	 * 
	 * 파라미터 {@code userId}는 seucirty 단과 {@link UserIDArgumentResolver}를 통해 {@link Authentication} 타입 오브젝트에 대한 
	 * 유효성 검사가 수행된 후 전달되기 때문에 안전하다고 판단하고 여기선 따로 같은 동작(null 검사 등)을 반복하지는 않는다. 만약 무언가 잘못되어 null 값의 
	 * 파라미터 {@code userId}가 전달 된다고 하여도 메서드의 시작부에서 {@link NullPointerException}이 발생할 것이므로 잘못된 정보가 계속 전달되지는 
	...
	public ResponseEntity<Void> deleteBookMark(@PathVariable("marketId")int marketId, UserID userId){
		...
		if(logger.isDebugEnabled()) {
			logger.debug("Calling {} to delete bookmark info for user id : {} started.", bookMarkService, userId.getUserId());
		}
