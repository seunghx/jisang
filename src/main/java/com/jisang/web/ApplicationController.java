...
public class ApplicationController {
    ...
    /**
     * 지상 어플리케이션 클라이언트와 서버 어플리케이션에서 사용되는 코드의 리스트들을 내부 프로퍼티로 담고 있는 {@link CodeBook}의
     * 오브젝트를 응답으로 전달한다.
     */
    ...
    /**
     * {@link AuthenticationNumberEntryPointFilter}는 인증 번호 발급 전 인증 번호를 발급하려는 목적지 정보에
     * 대한 검증 및 인증 번호 발급을 수행한다. 이 필터 클래스는 성공적인 인증 및 인증 번호 발급을 마치면 즉시 클라이언트에게 응답을 보낸다.
     * (컨트롤러를 거치지 않는다.) 그러므로 아래의 메서드는 실제로 호출되지는 않으나 API 문서 작성을 위해 선언하게 되었다.
     */
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "email", value = "인증 번호를 발급받으려는 유저 이메일", dataType = "string", paramType = "query", required = true),
            @ApiImplicitParam(name = "destination", value = "인증 번호를 발급받으려는 유저의 핸드폰 번호(현재).", dataType = "string", paramType = "query", required = true), })
    ...
    public ResponseEntity<Void> fakeGetAuthenticationNumber() {
        logger.error("This method {} should not be called.",
                getClass().getSimpleName() + "#" + "fackGetAuthenticationNumber().");
    ...
    public ResponseEntity<Void> fakePostAuthenticationNumber() {
        logger.error("This method {} should not be called.",
                getClass().getSimpleName() + "#" + "fackPostAuthenticationNumber().");
        return new ResponseEntity<>(HttpStatus.OK);
    }
