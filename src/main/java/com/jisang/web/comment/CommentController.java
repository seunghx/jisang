...
public class CommentController {
    ...
    /**
     * 
     * 현재 지상 어플리케이션에서 허용하는 댓글 깊이(댓글에 대한 댓글)를 초과한 깊이의 댓글이 발견될 경우
     * {@link NestedCommentException}이 던져진다. 아래 메서드는 이 예외를 처리한다.
     *
     */
    ...
    public ResponseEntity<ErrorDTO> onNoSuchParentCommentId(NoSuchParentCommentIdException ex, WebRequest request) {
        ...
        ErrorDTO errDTO = new ErrorDTO(HttpStatus.BAD_REQUEST.value(),
                msgSource.getMessage("response.exception.NoSuchParentCommentIdException", null, request.getLocale()));
    ...
    /**
     * 
     * 지상 어플리케이션 상에 존재하지 않는 상품(id)에 대한 댓글 등록 요청이 발견되었을 때
     * {@link DataIntegrityViolationException}이 발생할 수 있다. 아래 메서드는 이 예외를 처리한다.
     *
     */
    ...
    public ResponseEntity<ErrorDTO> onDataIntegrityViolation(DataIntegrityViolationException ex, WebRequest request) {
        ...
        ErrorDTO errDTO = new ErrorDTO(HttpStatus.BAD_REQUEST.value(), msgSource
                .getMessage("response.exception.DataIntegrityViolationException.comment", null, request.getLocale()));
     ...
     * 
     * url이 {@code /product/{productId}/comment}이긴 하나 댓글은 댓글 내용 외에도 작성자 id, 상품 id 등을
     * 포함하므로 댓글을 하나의 도메인으로 여기게 되었다. 그러므로 해당 url을 아래 메서드를 통해 이 클래스
     * {@link CommentController}에서 처리한다.
     * 
     */
