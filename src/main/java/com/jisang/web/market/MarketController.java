...
public class MarketController {
	...
	/**
	 * 
	 * 마켓 정보 수정을 담당하는 핸들러 메서드이다. {@link PutMapping}을 사용하지 않은 이유는, 그러니까 해당 요청의 HTTP 메서드를 PUT이 아닌
	 * POST로 정한 이유는 멀티파트 파일의 업로드가 필요하기 때문이다. 또한 수정 작업임에도 응답 상태 코드가 204- No Content가 아닌 201-Created인 
	 * 이유는 변경된 정보를 클라이언트에 전달해야 하기 때문이다.
	 * 
	 * 파라미터 {@code userId}는 seucirty 단과 {@link UserIDArgumentResolver}를 통해 {@link Authentication} 타입 오브젝트에 대한 
	 * 유효성 검사가 수행된 후 전달되기 때문에 안전하다고 판단하고 여기선 따로 같은 동작(null 검사 등)을 반복하지는 않는다. 만약 무언가 잘못되어 null 값의 
	 * 파라미터 {@code userId}가 전달 된다고 하여도 메서드의 시작부에서 {@link NullPointerException}이 발생할 것이므로 잘못된 정보가 계속 전달되지는 
	...
	public ResponseEntity<MarketManagementDTO> modifyMarket(UserID userId, @Validated MarketModifyRequestDTO marketDTO){
		int managerId = userId.getUserId();
