...
/**
 * 
 * 지상 어플리케이션 와이어프레임상의 마켓 관리 화면들에 사용될 DTO 클래스들이 내부적으로 정의되어있으며 이들 정적 내부 클래스에 공통으로 사용될 프로퍼티들이
 * 정의되어 있다. 처음에는 상품 등록 및 수정 과정에 필요한 모든 요청/응답에 하나의 DTO 클래스를 이용하려고 하였다. 그러나 springfox에 {@link @JsonView} 또는
 * {@link ApiModelProperty}의 {@code readOnly} 와 {@code accessMode.READ_ONLY} 어트리뷰트가 제대로 동작되지 않아 DTO 클래스를 상황에 맞게 나누게 
 * 되었다. 
 * 
 * 
 * @author leeseunghyun
...
public class ProductManagementViewDTO {
	...
	@ApiModelProperty(notes = "상품 카테고리 코드. ", name = "category", required = true, allowableValues = "range[30, 38]"
					, example = "OUTER : '31", value = "코드 북에 지정된 카테고리 코드를 사용해야 한다.")
	...
	public String toString() {
		return getClass() + getName() + "[category=" + category + ", name=" + name + ", price=" + price + ", detail=" + detail
	...
	public abstract static class ProductMutationDTO extends ProductManagementViewDTO {
		...
		@ApiModelProperty(notes = "상품 대표 이미지.", name = "representingImage", required = true
				, value = "jpg | png | tif 등의 올바른 이미지 파일 포맷이어야 한다. 해당 프로퍼티는 상품 수정 화면에 대한 응답시에는 포함되지 않는다.")
		@ImageExtension(message = "올바른 이미지 파일 형식이 아닙니다.")
		private MultipartFile representingImage;

		@ApiModelProperty(notes = "상품 이미지.", name = "images", required = true
				, value = "jpg | png | tif 등의 올바른 이미지 파일 포맷이어야 한다. 해당 프로퍼티는 상품 수정 화면에 대한 응답시에는 포함되지 않는다.")
	...
	/**
	 * 
	 * 상품 정보 수정 요청에 필요한 구성 정보를 담은 DTO 클래스이다. 현재 구현으로는 상품 수정과 등록에 다른 구성 정보가 존재하지는 않지만 후에 이런 부분은 충분히
	 * 추가될 수 있다고 생각한다. 에를들어, EAN-13(국제 상품 번호)은 상품 등록에는 필요한 정보일 수 있으나 수정에는 적절치 않은 정보이다.
	 * 
	 * 
	 * @author leeseunghyun
	 *
	...
	/**
	 * 
	 * 상품 정보 등록 요청에 필요한 구성 정보를 담은 DTO 클래스이다. 현재 구현으로는 상품 수정과 등록에 다른 구성 정보가 존재하지는 않지만 후에 이런 부분은 충분히
	 * 추가될 수 있다고 생각한다. 에를들어, EAN-13(국제 상품 번호)은 상품 등록에는 필요한 정보일 수 있으나 수정에는 적절치 않은 정보이다.
	 * 
	 */
	...
	public static class ProductModifyResponseDTO extends ProductManagementViewDTO {
		...
		@ApiModelProperty(notes = "상품 대표 이미지의 url.", name= "representingImageUrl"
						, value = "해당 프로퍼티는 상품 수정 화면에 대한 응답에(만) 포함되는 정보이다.")
		private String representingImageUrl;
		
		@ApiModelProperty(notes = "상품 이미지들의 url.", name= "imageUrl"
						, value = "해당 프로퍼티는 상품 수정 화면에 대한 응답에(만) 포함되는 정보이다.")
