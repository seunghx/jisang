...
public class BestViewCriteria extends ProductListViewCriteria {
    ...
    /**
     * {@link BestViewCriteriaBuilder}에 대한 빌더 클래스이다. 이 클래스의 생성자 및 수정자 메서드에서 전달된 인자에
     * 대한 유효성 검사를 수행함을 알 수 있는데, 사실 이 클래스에 전달되는 값은 모두 컨틀롤러 단에서의 bean validation에 통과한
     * 값들이다. 그러나 혹시 모를 코드의 변경으로 인한 잘못된 값의 전달에 따른 뒷단의 오동작을 방지하고자 검사를 하였다. 확실하게 안전함을
     * 보장할 수 있기 때문이다. 그러나, 테스트에 거쳐 코드 문제가 없음을 알며 동시에 bean validation을 통과하였음까지 아는
     * 상황에서 또 한번 검사하는 것은 불필요한 것은 아닌가란 생각도 들어 정답을 모르겠다.
     */
    public static class BestViewCriteriaBuilder extends ProductListViewCriteriaBuilder {
        ...
        public BestViewCriteriaBuilder(Integer marketId, String mallLocation, String category, int bestViewCnt) {
            ...
            if (this.bestViewCnt < 0) {
                logger.error("Invalid argument bestViewCnt detected : {}. bestViewCnt must not be negative value.",
                        bestViewCnt);
                logger.error(
                        "Because arguent bestViewCnt passed to bean validation, It must be caused by illegal code. Checking illegal code required.");
