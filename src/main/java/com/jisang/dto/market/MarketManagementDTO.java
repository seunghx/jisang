...
public class MarketManagementDTO {
	...
	@ApiModelProperty(notes = "지하상가 위치 코드.", name = "location", required = true
					, example = "강남 : '11'", value = "코드북에 지정된 위치 코드를 사용해야 한다.")
	...
	public static class MarketModifyResponseDTO extends MarketManagementDTO {
		...
		/**
		 * S3 등 스토리지 서비스에 저장된 파일의 이름을 나타내는 문자열 프로퍼티이다. 
		 * RDB에는 {@code imageFile}이 아닌 이 프로퍼티 {@code image}가 저장된다.
		 */
