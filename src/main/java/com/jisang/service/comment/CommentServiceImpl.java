...
public class CommentServiceImpl implements CommentService{
	...
	/**
	 * 
	 * 댓글 등록 로직을 수행한다. 이 메서드의 초반부에는 {@code commentDTO.parentId}에 대한 검사가 수행되고 있음을 알 수 있다. 이런 검사를 수행하는 이유는
	 * {@link NoSuchParentCommentIdException} 주석에서 다루었듯이 데이터베이스의 tbl_comments 테이블의 comment_parent_id 칼럼에 foreign key를
	 * (self reference) 지정하지 않았기 때문에 안전한 데이터의 삽입(안전이라기보단 올바르지 않은 데이터가 저장되는 것을 방지)이 가능하기 위함이다.
	 * 
	 * 또한 현재 구현은 댓글의 깊이가 2단계로 한정하여 구현이되어 있다. 이런 구조의 경우 후에 대댓글에 대한 댓글도 가능하다는 요구사항이 추가될 경우 메서드의 변경이
	 * 필요하다. 후에 코드를 수정해야겠다.
	 * 
	 * 존재하지 않는 상품 id가 입력될 경우 {@link DataIntegrityException}이 발생하며 이 예외는 보다 구체적인 예외인 {@link NoSuchProductException}
	 * 으로 변환된다. 이렇게 구체적인 예외를 사용할 경우 후에 이 클래스가 구현하는 {@link CommentService}에 로직이 추가되어 기존에는 없던 또 다른 
	 * {@link DataIntegrityViolationException}을 발생시킬 요소가 추가 될 경우에도 변경이 필요없다. 또한 특정 컨트롤러 클래스에서 예외를 처리하게 하지 않고
	 * {@link @ControllerAdvice} 클래스에 해당 예외에 대한 처리 로직을 등록하여 공통 처리하기에도 더 좋다. {@link DataIntegrityViolationException}의
	 * 경우 다른 도메인에서도 같은 예외가 던져질 수 있기 때문에 공통 처리하기에 좋지 않다.
	 * 
	 */
	@Override
	@Transactional
	public void registerComment(CommentRegisterDTO commentDTO) {
		...
		if(commentDTO.getParentId() != 0) {
			...
			if(parent == null) {
				...
				throw new NoSuchParentCommentIdException("Non existing parent comment id detected while trying to register comment"
				...
				throw new NestedCommentException("Illegal comment hierarchy detected while tryping to register comment");
			}
		}
		...
		try {
			...
			logger.info("Exception {} occured while trying to register comment. It is caused by non-existing product id.", e.toString());
