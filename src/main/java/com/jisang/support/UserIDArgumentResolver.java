 ...
 * 
 * 
 * 유저 정보 조회를 위해선 seucirty 단에서의 JWT 파싱으로 생성된 유저 정보가 필요하다. 이 정보는 {@link Authentication} 오브젝트를 통해서
 * 전달 받을 수 있는데, {@link Authentication} 타입 오브젝트가 예상과 다른 오브젝트일 경우 올바른 동작이 불가능하기 때문에 이를 검사하는게 안전다하고 
 * 생각하여 아래와 같이 구체적인 {@link Authentication} 구현 오브젝트에 대한 타입 검사를 수행한다.
 ...
 * 
 * 
 * 기존 인증이 필요한 url 처리를 담당하는 핸들러 메서드들에서는 {@link Authentication} 타입 파라미터를 전달 받은 후 아래와 같은 로직을 이용해
 * 유저의 id를 받아왔었다.
 * 
 * <pre>
 * 	int userId = Optional.ofNullable(authentication)
 *				   	 	 .filter(auth -> auth instanceof DefaultUserDetailsAuthentication)
 *				   		 .map(auth -> ((DefaultUserDetailsAuthentication)auth).getPrincipal().getId())
 *				   		 .orElseThrow(() -> {
 ...
 * </pre>
 * 
 * 매번 각각의 핸들러 메서드에서 위의 로직을 수행하기 때문에 코드의 중복이 많이 심했고 그런 이유로 {@link HandlerMethodArgumentResolver}를 구현한 
 * 이 클래스를 정의하게 되었다.
 * 
 * 
 * @author leeseunghyun
...
public class UserIDArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public boolean supportsParameter(MethodParameter parameter) {
		Objects.requireNonNull(parameter, "Null value method parameter detected while tyring to check paramter supportness");
	...
	/**
	 * @param parameter - {@link #supportsParameter}에 의해 {@link Integer} 타입임이 분명하며 null 검사도 이미 진행되었다.
	 */
	@Override
	public UserID resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		if(authentication instanceof DefaultUserDetailsAuthentication){
			...
			logger.error("Authentication must be instance of {} but {}"
					   						  			, DefaultUserDetailsAuthentication.class, authentication);
