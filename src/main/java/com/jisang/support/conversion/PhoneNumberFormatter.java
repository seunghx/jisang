...
public class PhoneNumberFormatter implements Formatter<String> {
    ...
    public String print(String formattedPhoneNumber, Locale locale) {
        ...
        return formatters.stream().filter(formatter -> formatter.supports(locale)).findFirst()
                .map(formatter -> formatter.print(formattedPhoneNumber)).orElseThrow(() -> {
                    if (logger.isInfoEnabled()) {
                        logger.info("Received unsupported Locale argument. Argument locale : {}.", locale);
                    }
                    throw new UnsupportedLocaleException("Unsupported Locale detected.",
                            TOPIC_NAME_FOR_UNSUPPORTED_LOCALE_EXCEPTION);
                });
    }
    ...
    public String parse(String rawPhoneNumber, Locale locale) throws ParseException {
        ...
        return formatters.stream().filter(formatter -> formatter.supports(locale)).findFirst()
                .map(formatter -> formatter.parse(rawPhoneNumber)).orElseThrow(() -> {
                    if (logger.isInfoEnabled()) {
                        logger.info("Received unsupported Locale argument. Argument locale : {}.", locale);
                    }
                    throw new UnsupportedLocaleException("Unsupported Locale detected.",
