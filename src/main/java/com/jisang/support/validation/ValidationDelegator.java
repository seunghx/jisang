...
/**
 * 
 * {@link Validator}의 메서드를 호출하여 bean validation을 수행한다. 검증 작업 중 예외가 발생하면 이에 대한
 * 로깅으로 어느 필드에 어느 문제가 있는지 알아야할 필요가 있는데, 빈 검증 작업을 호출하는 곳의 코드가 이러한 로깅때문에 더렵혀지는 것을
 * 방지하고 또한 코드의 중복을 완화시켜주기 위해 이 클래스를 작성하게 되었다.
 * 
 * @author leeseunghyun
 *
 */
        ...
        if (constraintViolations.size() > 0) {
            ...
            for (ConstraintViolation<T> violation : constraintViolations) {
                if (logger.isInfoEnabled()) {
                    ...
                    fieldErrors.add(new FieldError(obj.getClass().getSimpleName(),
                            violation.getPropertyPath().toString(), violation.getMessage()));
                }
            }

            throw new FieldErrorBasedValidationException("Invalid object detected. Invalid object :" + obj,
                    fieldErrors);
        }
