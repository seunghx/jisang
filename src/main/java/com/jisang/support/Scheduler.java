...
/**
 * 
 * 지상 어플리케이션에 필요한 스케줄링 관련 메서드는 다 이 클래스 아래에 두려고 한다. 
 * 현재 구현으로는 아래 {@link #refreshProductHit()}와 AWS S3 상의 이미지와 elasticsearch에 저장된 (검색어 미리보기로 사용되는) 해시태그 정보 중
 * 예외 발생 등의 이유로 삭제되지 않은 정보를 주기적으로 삭제하는 메서드 {@link #deleteImages(), #deleteHashTags()}가 존재한다.
 *  
 * 
 * @author leeseunghyun
 *
...
public class Scheduler {
	...
	/**
	 * 특정 기간마다 상품 조회수 정보를 업데이트 해줄 필요가 있다. 지상 어플리케이션에는 베스트 상품 목록 화면이 존재하는데, 상품 조회수를 업데이트 해주지 않을 경우 
	 * 17FW의 인기 제품이 18SS의 베스트 상품 뷰에 여전히 남아있을 수 있다.
	 */
